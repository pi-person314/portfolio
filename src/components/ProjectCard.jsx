...
export default function ProjectCard({title, tech, desc, image, darkMode}) {
    ...
    return (
        <div className={`flex flex-col justify-between items-center shadow-xl rounded-xl space-y-8 p-10 ${darkMode ? "bg-zinc-700" : "bg-zinc-400"}`}>
            <h1 className="text-center text-2xl lg:text-3xl font-serif"><b>{title}</b></h1>

            {details ? 
                <div className="flex flex-col justify-center space-y-4 h-52">
